...
The system provides 8 specialized tools accessible through the AI agent:

#### 1. **find_customer**
Find customers by various criteria (name, email, phone, segment, behavior).
...
#### 2. **find_card_product**
Search for card products by type, name, or network.
...
#### 3. **recommend_card_products**
Get AI-powered card recommendations for a specific customer.
...
#### 4. **recommend_customers**
Find the best customers for a specific card product.
...
#### 5. **find_rm_task**
Search for tasks by status, priority, or date range.
...
curl -X POST http://localhost:8000/chat/stream \
  -H "Content-Type: application/json" \
  -d '{
    "message": "Hiệu suất của tôi tháng này như thế nào?",
    "thread_id": "user-123",
    "rm_id": 1
  }'
```
...
curl -X POST http://localhost:8000/chat \
  -H "Content-Type: application/json" \
  -d '{
    "message": "Tạo task cho khách hàng Thắng",
    "thread_id": "user-123",
    "rm_id": 1
  }'

# Response will include: "has_interrupt": true
...
- VPBank for organizing the Hackathon 2025
...
- Contact the team through the hackathon platform
- Review detailed documentation in each component's README
